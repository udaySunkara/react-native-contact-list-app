      ...
      <Stack.Screen
        ...
        options={{
          title: 'Contact list', headerRight: () => {
            return <TouchableOpacity onPress={() => { if (isEditmodeOn) return; navigate('Manage', { title: 'New contact' }) }}>
              <AntDesign name="pluscircle" size={24} color={isEditmodeOn ? '#cfcfcf' : '#00AA55'} />
            </TouchableOpacity>
          },
          headerLeft: () => {
          return <>{contactList && contactList.length > 0 ? <TouchableOpacity onPress={() => enableEditView()} style={styles.headerLeft}>{ !isEditmodeOn ? <Text maxFontSizeMultiplier={1.5}>Select</Text>: <Text maxFontSizeMultiplier={1.5}>Undo</Text>}</TouchableOpacity> : null}</>
          },
